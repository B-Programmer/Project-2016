    ...
    public partial class FrmChangeAdminPassword : Form
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                if ((isAllEntryFilled()) && (isRegistered(txtUsername.Text)))
                {
                     ...
                     */
                    cmd.CommandText = "UPDATE tblAdminUser SET [Password] = '" + Password + "' WHERE (Username = '" + Username + "')";
                    cmd.Connection = cn;
                    cn.Open();
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Authorized User's Password has been successfully Changed!!!", "Car Rental App.", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cn.Close();
                }
            }
            catch (Exception er)
        ...
        private bool isRegistered(string user)
        {
            ...
            try
            {
                ...
                if (!dr.HasRows)
                {
                    MessageBox.Show("Fail to change password!!! Username not exist. No user found ", "Car Rental App.", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void FrmChangeAdminPassword_Load(object sender, EventArgs e)
        {
            cmd.Connection = cn;
        }
    }
